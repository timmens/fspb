...
@pytest.fixture
def data():
    # 4 samples, 3 features, 2 time points
    # Need n_samples > n_features to avoid division by zero in error covariance
    x = np.zeros((4, 3, 2))
    x[:3, :, :] = np.stack([np.eye(3), 2 * np.eye(3)], axis=2)
    # 4th sample is zeros (doesn't affect residuals.T @ residuals sum)
    x_new = 3 * np.eye(3)[:, :2]
    residuals = np.array(
        [
            [-1, -2],
            [0, 0],
            [1, 2],
            [0, 0],  # 4th sample with zero residuals
        ]
    )
    ...
    for s in (0, 1):
        for t in (0, 1):
            if s == t == 0:
                ...
                arr = np.eye(3) / 2
            expected[s, t] = 4 * arr  # 4 is the number of samples
...
def test_calculate_homoskedastic_error(data):
    ...
    # n_parameters = 2
    # Denominator = len(residuals) - n_parameters = 4 - 2 = 2
    expected = np.array(
        [
            [1, 2],
            [2, 4],
...
def test_multiply_x_new_sigma_x_inv_x_newT(data):
    ...
    # With 4 samples, sigma_x_inv scales by 4/3 compared to 3 samples
    # Old values [[27, 0], [0, 6.75]] become [[36, 0], [0, 9]]
    expected = np.array(
        [
            [36, 0],
            [0, 9],
...
def test_calculate_covariance_on_grid(data):
    ...
    # sigma_error = [[2, 4], [4, 8]] (from error covariance: residuals.T @ residuals / (4-3))
    # xt_sigma_x_inv_xt = [[36, 0], [0, 9]] (from multiply test with 4 samples)
...
def test_estimate_scaling_covariance_and_dof(data):
    ...
    )  # [[1, 2], [2, 4]] with 4 samples: [[2,4],[4,8]] / (4-2)
    dof = estimate_dof(residuals)
    expected = sigma_error * (dof - 2) / dof
...
def test_calculate_covariance_prediction_band_internal(data):
    ...
    # Note: must use x.shape[1] for n_parameters to match actual function behavior
    ...
    scaling_cov = _estimate_scaling_covariance_and_dof(
        residuals, n_parameters=x.shape[1]
    )
...
def test_calculate_covariance_confidence_band_homoskedastic_direct(data):
    ...
    x, x_new, residuals = data
    expected = np.array([[72.0, 0.0], [0.0, 72.0]])
...
def test_multiply_functions(data):
    ...
    # Verify these are the expected values from our data (with 4 samples)
    expected_aba = np.array([[36.0, 0.0], [0.0, 9.0]])
