...
from fspb.config import SRC, BLD_APPLICATION, USE_CLEANED_DATA, USE_ANONYMIZED_DATA


def get_covariates_path(use_cleaned_data: bool, use_anonymized_data: bool) -> Path:
    if use_cleaned_data and use_anonymized_data:
        return SRC / "application" / "covariates_anonymized.csv"
    elif use_cleaned_data and not use_anonymized_data:
        return SRC / "application" / "covariates.csv"
    elif not use_cleaned_data and use_anonymized_data:
        raise ValueError("Anonymized data is only available in cleaned form.")
    else:
        return SRC / "application" / "CovariatesReduced.csv"


def get_outcomes_path(use_cleaned_data: bool, use_anonymized_data: bool) -> Path:
    if use_cleaned_data and use_anonymized_data:
        return SRC / "application" / "outcomes_anonymized.csv"
    elif use_cleaned_data and not use_anonymized_data:
        return SRC / "application" / "outcomes.csv"
    elif not use_cleaned_data and use_anonymized_data:
        raise ValueError("Anonymized data is only available in cleaned form.")
    else:
        return SRC / "application" / "FRONT_V.csv"
...
for amputee in (False, True):
    ...
    x_data_path = get_covariates_path(USE_CLEANED_DATA, USE_ANONYMIZED_DATA)
...
def _clean_predictors(df_x: pd.DataFrame, is_clean: bool) -> pd.DataFrame:
    ...
    clean["block_time"] = df_x["TPush_V"].astype(pd.Float64Dtype())
    return clean
...
for amputee in (False, True):
    ...
    y_data_path = get_outcomes_path(USE_CLEANED_DATA, USE_ANONYMIZED_DATA)
